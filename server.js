...
async function handleAPI(req, res) {
  if (req.method === 'GET') {
    ...
    res.end(JSON.stringify({ version: 'railway-v3', ok: true }));
    return;
  }
  if (req.method !== 'POST') {
  ...
  req.on('end', async () => {
    try {
      ...
      console.log('API call, webSearch:', useWebSearch);

      // Build request body based on whether web search is needed
      const requestBody = {
      ...
      // Only add web_search tool when needed (etapas 1 e 2)
      const headers = {
...
server.listen(PORT, '0.0.0.0', () => {
  console.log('TripMind railway-v3 port', PORT, 'key:', !!ANTHROPIC_KEY);
});
